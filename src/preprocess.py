...
class DataMaker():
    ...
    def __init__(self) -> None:
        ...
        self.data_name = "penguins_lter"#self.config["DATANAME"]["name"]
        self.project_path = os.path.join(os.getcwd(), "data")
        self.data_path = os.path.join(self.project_path, self.data_name + ".csv")
        self.X_path = os.path.join(self.project_path, self.data_name + "_X.csv")
        self.y_path = os.path.join(self.project_path, self.data_name + "_y.csv")
        self.train_path = [os.path.join(self.project_path, "Train_" + self.data_name + "_X.csv"), os.path.join(
            self.project_path, "Train_" + self.data_name + "_y.csv")]
        self.test_path = [os.path.join(self.project_path, "Test_" + self.data_name + ".csv"), os.path.join(
            self.project_path, "Test_" + self.data_name + "_y.csv")]
        self.log.info("DataMaker is ready")

    def get_data(self) -> bool:
        dataset = pd.read_csv(self.data_path).drop(['studyName', 'Sex', 'Region', 'Island', 'Stage', 'Clutch Completion', 'Date Egg', 'Individual ID', 'Sample Number', 'Comments'], axis=1).dropna()
